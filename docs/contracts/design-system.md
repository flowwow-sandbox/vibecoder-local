...
> Область применения — маркетинговые лендинги и промо-страницы. Продуктовые интерфейсы (клиентский веб, B2B-кабинет корпоративного клиента, админка селлера) описаны отдельными контрактами — см. таблицу «Дизайн: какую поверхность делаешь» в `AGENTS.md`.
...
|---|---|---|---|
| `--ff-color-coral` | `#FF7864` | `255, 120, 100` | Акценты, крупные цветовые блоки |
...
| Токен | HEX | RGB | Назначение |
|---|---|---|---|
| `--ff-color-deep-purpur` | `#370A28` | `55, 10, 40` | Текст, логотип, тёмные секции, footer, primary CTA |

> Это **не чёрный**. Всегда используй `#370A28` вместо `#000` / `#000000` для текста и тёмных поверхностей — это даёт фирменную «тёплую» темноту.
...
| `--ff-color-text` | `#370A28` | Основной текст |
...
module.exports = {
  theme: {
    extend: {
      colors: {
        ff: {
          coral:        { DEFAULT: '#FF7864', 4: '#FF9486', 3: '#FFAFA4', 2: '#FFC9C2', 1: '#FEE2DF' },
          'deep-purpur':'#370A28',
          'soft-pink':  '#FFE9EE',
          'comfy-beige':'#FFF1E5',
          'soft-purple':'#F1ECFF',
...
Для Tailwind v4 (`@theme`) — те же значения через CSS-переменные в директиве `@theme { --color-ff-coral: #FF7864; ... }`. Для styled-components / Emotion — экспортируй объект `tokens` из единого модуля и не дублируй HEX по компонентам.
...
- Токены этого файла не смешиваются с токенами продуктовых контрактов (`client-web-rules.md`, `seller-admin-web-rules.md`). Coral / Deep Purpur, radius `40px` и отсутствие теней — лендинговый набор; Grey-шкала, radius `12px` и тени у попапов — продуктовый. Один интерфейс — один набор.
- Тёмной темы нет — все лендинги только светлые.
- Тени отсутствуют на всех элементах: ни кнопки, ни карточки, ни изображения.
- Coral (`#FF7864`) не используется как цвет body-текста.
- Deep Purpur (`#370A28`) не заменяется на `#000000`.
