...
- **Временный файл загрузки задавай явно — рядом с данными проекта.** Парсер multipart кладёт тело во временный файл системного temp-каталога (Starlette/FastAPI — всегда, всё что больше 1 МБ; `@fastify/multipart` — по явной просьбе, `saveRequestFiles()`). Системный temp бывает заметно меньше принимаемого файла, и тогда загрузка падает на записи (`No space left on device`) ещё до твоего кода — симптом выглядит случайным сбоем, а не превышением лимита. Указывай каталог сам (`TMPDIR` или настройка фреймворка), рядом с `app/data/`, и **создавай его при старте**: на несуществующий `TMPDIR` Python молча берёт системный `/tmp` обратно — настройка будет выглядеть сделанной, не работая. `TMPDIR` управляет только теми, кто пишет через системный temp: у busboy своих хелперов нет — место записи выбираешь сам в pipeline, а `toBuffer()` держит файл в памяти и на диск не смотрит вовсе.
- **Свой потолок на приём и уборка временных файлов — на тебе.** Перенеся временный каталог к данным проекта, ты снял ограничитель, которым раньше служил маленький системный temp. Ставь явный лимит на размер запроса, проверяй, что парсер не обрезал данные молча, и убирай недогруженные остатки.
...
### 3.1 Обход хуков: единственный путь и почему он такой

Обойти guard можно **только** env-переменной (`CLAUDE_ALLOW_DESTRUCTIVE`, `CLAUDE_ALLOW_GIT_DESTRUCTIVE`, `CLAUDE_ALLOW_SECRETS`, `CLAUDE_ALLOW_WORKDIR`, `CLAUDE_ALLOW_INJECTION`), выставленной **в окружении, из которого запущен агент-тул**. **На пилоте использовать её ЗАПРЕЩЕНО.**

Почему путь именно такой: хук стартует как sibling-процесс агент-харнесса, а не как потомок bash-команды. Поэтому inline-префикс `CLAUDE_ALLOW_X=1 <command>`, который агент может написать сам, хуку **не виден** — переменную выставляет только человек, до запуска тула. Обход намеренно вынесен за пределы досягаемости агента.

- ❌ `CLAUDE_ALLOW_*=1 <command>` — не сработает (и запрещено).
- ❌ `export CLAUDE_ALLOW_*=1` в окружении тула — запрещено политикой.
- ❌ `disableAllHooks: true` в `~/.claude/settings.json` — отключение хуков глобально.
- ❌ Правка `.claude/hooks/**`, `.claude/settings.json`, `.codex/hooks.json` ради снятия блока.

**Маркера-обхода в тексте команды не существует.** Раньше хуки понимали комментарий вида `# claude-bypass: <name>` прямо в команде — это удалено намеренно: команду пишет агент, и такой маркер означал бы, что агент выписывает себе разрешение сам, а защита декоративна. Если увидишь этот синтаксис в старом примере — он больше ни на что не влияет.

`git-no-verify-guard` bypass'а не имеет вообще — ни через env, ни как-либо ещё.

Если хук блокирует то, что ты считаешь legitimate-операцией — остановись и обратись к ответственному за платформу, не обходи.
...
| `git rebase -i HEAD~N` | ❌ Blocked — концептуально сложно для non-tech, легко всё сломать; force-push блокирован → испорченная локальная история всё равно не доедет до remote | git-destructive-guard |
...
| `git push --force` / `--force-with-lease` | ❌ Blocked (обе формы: «вежливый» вариант ломает базу diff'а для cross-review так же) | git-destructive-guard |
| `git branch -D <name>` | ❌ Blocked при прямом вызове¹ | git-destructive-guard |
...
¹ `make wt-prune` удаляет локальную ветку сам — но только у worktree, чья origin-ветка уже удалена и чей tip подтверждён merged-PR через `gh`. Хук на это не срабатывает (команда выполняется внутри Makefile, а не в Bash-вызове агента), и это осознанно: удаляется ветка, уже влитая в main. Ручной `git branch -D` остаётся заблокированным.
...
- **Destructive-SQL во внешней БД** (`DROP TABLE` / `TRUNCATE` / `DELETE FROM` без `WHERE`, `dropdb`, `flushall`), отправленный через `psql` / `mysql` / `mongosh` / `redis-cli` и подобные клиенты — destructive-command-guard блокирует. Локальная SQLite проекта под это правило не подпадает: `sqlite3 app/data/app.db "DROP TABLE …"` — обычная dev-миграция, она проходит, и чтение собственных `.sql`-файлов (`grep`, `cat`) тоже.
