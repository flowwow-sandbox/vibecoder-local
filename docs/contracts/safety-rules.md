...
Guard-хуки ведут журнал срабатываний в **`~/.claude/logs/safety.log`** — вне репо, по одной JSONL-записи на событие: время, какой хук, вердикт (`deny` / `bypass` / `found`), сработавший паттерн и цель (команда или путь).

Что нужно знать:

- **Секреты в лог не попадают.** Значения, похожие на ключи и токены (`sk-…`, `ghp_…`, `AKIA…`, `Bearer …`, приватные ключи, JWT), заменяются на метку вида `[OpenAI API key redacted]` — и в записях guard'ов, и в записях детектора утечек. Частично редактированный вид ключа детектор печатает только в stderr, чтобы человек понял, что ротировать.
- **Файл не ротируется** — он растёт, пока его не удалить. Удалять можно в любой момент: на работу хуков это не влияет.
- **Лог локальный** — никуда не отправляется, лежит в домашнем каталоге и защищён только правами на него.

Если нужно понять, почему хук что-то заблокировал, — смотри последние строки этого файла, там сработавший паттерн.
